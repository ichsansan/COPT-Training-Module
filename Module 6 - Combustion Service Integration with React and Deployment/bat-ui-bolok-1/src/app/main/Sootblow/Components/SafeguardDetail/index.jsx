...
const SafeguardDetail = ({ open, close }) => {
    ...
    const { loading, sootblowSafeguardLabel,
        filterStartDate,
        filterEndDate,
        loadingExport,
    } = sootblowReducer;
    ...
    return (
        <Dialog maxWidth="md" fullWidth open={open} aria-labelledby="responsive-dialog-title">
            <AppBar position="static" className={classes.appBar}>
                <Toolbar>
                    ...
                    <Typography className="text-16 my-24 mx-16 flex-grow text-right" id="responsive-dialog-title">
                        <code>{!loading ? sootblowSafeguardLabel ? sootblowSafeguardLabel : 'Safeguard Detail' : 'Safeguard Detail'}</code>
                    </Typography>
                    {activeTab === 1 &&
            ...
            />}
            <Confirmation confirmText={loadingExport ? 'Exporting' : 'OK'} loading={loadingExport} open={openExportConfirmation} title='Export Safeguard History Data' contentText='Confirm to export sootblow recommendation history data?' cancelHandler={closeExportConfirmation} confirmHandler={confirmExportHandler} />
        </Dialog>
    )
}
