...
import { changeCombustion, exportCombustionSafeguardHistory } from 'app/store/actions';
...
const SafeguardDetail = ({ open, close }) => {
    ...
    const confirmExportHandler = async () => {
        const isSuccess = await dispatch(exportCombustionSafeguardHistory())
        if (isSuccess) {
    ...
    return (
        <Dialog maxWidth="md" fullWidth open={open} aria-labelledby="responsive-dialog-title">
            <AppBar position="static" className={classes.appBar}>
                <Toolbar>
                    ...
                    <Typography className="text-16 my-24 mx-16 flex-grow text-right" id="responsive-dialog-title">
                        <code>{!loading ? combustionSafeguardLabel ? combustionSafeguardLabel : 'Safeguard Detail' : 'Safeguard Detail'}</code>
                    </Typography>
                    {activeTab === 1 &&
                        <Tooltip title="Export Safeguard History Data" arrow>
                            <span>
                                <IconButton aria-label="export safeguard  history data" disabled={loading} onClick={() => setOpenDateFilter(true)}>
                                    <CloudDownload />
                                </IconButton>
                            </span>
                        </Tooltip>
            ...
            {openDateFilter && <DateRangeModal
                format='YYYY-MM-DD'
                views={["date"]}
                title={'Please select a time range to export safeguard  history data'}
                filterStartDateValue={filterStartDate}
                filterEndDateValue={filterEndDate}
                changeFilterStartDate={(e) => dispatch(changeCombustion({ filterStartDate: e }))}
                changeFilterEndDate={(e) => dispatch(changeCombustion({ filterEndDate: e }))}
                loading={false}
            ...
            />}
            <Confirmation confirmText={loadingExport ? 'Exporting' : 'OK'} loading={loadingExport} open={openExportConfirmation} title='Export Safeguard History Data' contentText='Confirm to export combustion safeguard  history data?' cancelHandler={closeExportConfirmation} confirmHandler={confirmExportHandler} />
        </Dialog>
    )
}
