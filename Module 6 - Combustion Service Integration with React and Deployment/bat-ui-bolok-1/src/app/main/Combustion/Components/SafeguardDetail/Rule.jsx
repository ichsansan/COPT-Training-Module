...
const Rule = () => {
	...
	const {
		...
		// combustionSuggestionAsTestData,
		loading,
		// loadingCombustionSuggestionAsTest,
		errorSafeguardDetailRule,
	} = combustionReducer;

	// const { area } = combustionSuggestionAsTestData

	const ruleData = combustionSafeguardDetailRuleData?.map(item => createRuleData(item.sequence, item.minTime, item.setValue, item.actualValue, item.tagDescription, item.status));
	...
	return (
		<>
						...
						: ruleData?.length === 0 ? (
							...
							<div className='flex flex-col py-12 gap-20'>
								<div className="flex flex-col">
									...
									{showRuleLogic &&
										<pre className='mt-12 '>
											<code style={{ color: combustionSafeguardDetailRuleValue ? '#4CAF50' : '#F44336' }} className={`whitespace-pre-line `}>
												{combustionSafeguardDetailRuleLogic}
											</code>
										</pre>
									}
			...
			{/* <Confirmation loading={loadingCombustionSuggestionAsTest} open={openConfirmation} title={`Confirm to add suggestion as test?`} contentText={`Make sure and to confirm this suggestion on area ${area} with trigger "Test" added`} cancelHandler={closeAddSuggestionConfirmation} confirmHandler={confirmAddSuggestion} loadingText={'Processing'} /> */}
		</>
	);
};
