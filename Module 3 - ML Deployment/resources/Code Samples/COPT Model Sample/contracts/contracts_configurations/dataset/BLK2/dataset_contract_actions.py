...
TAG_ASSOCIATIONS = {
    ...
    "Total Secondary Air Flow Sides Sum": lambda df_before, df_after:
        df_before["A2FT107_A"].values + \
        df_before["A2FT107_B"].values,

    # Single associations
    "Efficiency Nominal": {"Efficiency": None},
    "Excess Oxygen Sensor": {"A2AT102": None},
    "Windbox-to-Furnace Diff. Press A": {"A2PdT134A": None},
    "Windbox-to-Furnace Diff. Press B": {"A2PdT134B": None},
    ...
    "Generator Gross Load": {"A2G_Power": None},
    "Total Primary Air Flow": {
        "A2FT104_A": lambda x, scale_back=False: x * 2 if scale_back else x * 0.5,
        "A2FT104_B": lambda x, scale_back=False: x * 2 if scale_back else x * 0.5,
    },
    "Furnace Pressure": {
        "A2PT119A": None,
        "A2PT119B": None
